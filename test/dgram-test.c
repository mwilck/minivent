...
static int clt_cb(struct event *evt, uint32_t events)
{
	...
	if (evt->reason == REASON_TIMEOUT && evt->ep.events & (EPOLLIN|EPOLLOUT)) {
		...
		if ((rc = sendto(evt->fd, buf, n + 1, 0,
				 (const struct sockaddr *)&minivent_sa,
				 sizeof(minivent_sa))) != n + 1) {
			msg(LOG_ERR, "sendto: %d (%m), expected %d\n", rc, n + 1);
			stop_client(clt);
		}
...
static int client(int num)
{
	...
	snprintf(clt_addr.sun_path + 1, sizeof(clt_addr.sun_path) - 1,
		 "%s#%ld", minivent_sa.sun_path + 1, (long)clt.pid);

	if ((rc = bind(sfd, (struct sockaddr *)&clt_addr, sizeof(clt_addr))) == -1) {
...
static int start_clients(struct dispatcher *dsp)
{
	...
	for (i = 0; i < echo_cfg.n_clients; i++) {
		...
		if (!tim)
			return -ENOMEM;
		*tim = TIMER_ON_HEAP(start_clt, dsp, CLT_DELAY_SECS * 1000000 +
				     (random() % 11) * 1000);
...
static int server_cb(struct event *ev, uint32_t events)
{
	struct echo_event *echo = container_of(ev, struct echo_event, e);
	int rc;
	const struct timespec *new_tmo = { 0, };
	...
	if (ev->reason == REASON_TIMEOUT) {
		...
		memset(&echo->addr, 0, sizeof(echo->addr));
		len = sizeof(echo->addr);
		rc = recvfrom(ev->fd, echo->buf, sizeof(echo->buf), 0,
			      (struct sockaddr*)&echo->addr, &len);
		if (rc <= 0) {
			msg(LOG_ERR, "recvfrom: %m\n");
			return EVENTCB_CLEANUP;
		}
		ev->ep.events = EPOLLOUT|EPOLLHUP;
		new_tmo = &send_tmo;
	} else if (events & EPOLLOUT) {
		rc = sendto(ev->fd, echo->buf,
			    strnlen(echo->buf, sizeof(echo->buf)), 0,
			    (struct sockaddr *)&echo->addr, sizeof(echo->addr));
		if (rc == -1) {
...
static void kill_server(void* arg __attribute__((unused)))
{
	kill(getpid(), SIGINT);
}
...
static int server(void)
{
	...
	struct echo_event srv_event = { .e.fd = -1, };
	struct timer_event tim = { .e.fd = -1, };
	int rc;
	sigset_t mask;
	...
	srv_event.e = EVENT_W_TMO_ON_STACK(server_cb, fd, EPOLLIN,
					   1000000 * RECV_TMO_SECS);
	if ((rc = event_add(dsp, &srv_event.e) < 0))
		return rc;
	/* prevent __cleanup__ from closing, do this in cleanup() cb */
	fd = -1;

	tim = TIMER_ON_STACK(kill_server, NULL, echo_cfg.runtime * 1000000);
	if (event_add(dsp, &tim.e) < 0)
		msg(LOG_ERR, "failed to add stop timer\n");
...
static int parse_opts(int argc, char * const argv[])
{
	...
	while ((opt = getopt_long(argc, argv, opts, longopts, NULL)) != -1) {
		...
		case 'n':
			...
			break;
		case 't':
			read_int(optarg, "--runtime", &echo_cfg.runtime);
			break;
		case 'w':
	...
	if (echo_cfg.runtime <= 0) {
		msg(LOG_ERR, "runtime must be positive\n");
		return -EINVAL;
	} else
		accept_tmo.tv_sec = echo_cfg.runtime;
